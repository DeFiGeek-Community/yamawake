...
contract LiquidityGauge is ReentrancyGuard {
    ...
    constructor(address lpToken_) {
        ...
        string memory _symbol = ERC20Extended(lpToken_).symbol();
        ...
        inflationParams =
            (CRV20(CRV).futureEpochTimeWrite() << 216) +
            CRV20(CRV).rate();
    ...
    function _checkpoint(address addr) internal {
        ...
        if (block.timestamp > _st.periodTime) {
            ...
            Controller(GAUGE_CONTROLLER).checkpointGauge(address(this));
            ...
            for (uint256 i = 0; i < 500; i++) {
                ...
                uint256 w = Controller(GAUGE_CONTROLLER).gaugeRelativeWeight(
                    address(this),
    ...
    function _updateLiquidityLimit(
        ...
        uint256 _votingBalance = VotingEscrowBoost(VEBOOST_PROXY)
    ...
    function kick(address addr_) external {
        ...
        uint256 _tVe = VotingEscrow(VOTING_ESCROW).userPointHistoryTs(
            addr_,
            VotingEscrow(VOTING_ESCROW).userPointEpoch(addr_)
        );
    ...
    function setRewardDistributor(
        ...
        require(
            msg.sender == currentDistributor ||
                msg.sender == Factory(factory).admin()
        );
    ...
    function claimableTokens(address addr_) external returns (uint256) {
        ...
        return
            ...
            Minter(MINTER).minted(addr_, address(this));
    }
    ...
    modifier onlyAdmin() {
        require(msg.sender == Factory(factory).admin());
        _;
    }
}
