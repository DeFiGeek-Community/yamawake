...
export async function addTemplate(networkName: string, templateName: string, deployedFactoryAddress: string, deployedTemplateAddress: string):Promise<string>{
    /*
    ...
    */
    ...
    let tx = await Factory.connect(foundation).addTemplate(name, Template.address,initializeSignature, transferSignature,  {gasLimit: 10000000})
    await tx.wait();
