...
describe("YMWK", function () {
  ...
  describe("constructor", function () {
    ...
    it("constructor_success_1", async function () {
      ...
      await expect(await ymwk.totalSupply()).to.be.equal(ethers.utils.parseEther("450000000"));
  ...
  describe("set_minter", function () {
    ...
    it("set_minter_fail_1", async function () {
        ...
        await expect(ymwk.connect(addr1).set_minter(addr1.address)).to.be.reverted;
    });
    ...
    it("set_minter_success_2", async function () {
        ...
        await expect(await ymwk.minter()).to.be.equal(ethers.constants.AddressZero);
    });
  });
  ...
  describe("set_admin", function () {
    ...
    it("set_admin_success_1", async function () {
      ...
      it("set_admin_fail_2", async function () {
          ...
          await expect(await ymwk.admin()).to.be.equal(ethers.constants.AddressZero);
      });
    });
    ...
    describe("mint", function () {
        ...
        it("mint_success_1", async function () {
            ...
            await expect(await ymwk.connect(addr1).mint(addr1.address, amount)).to.be.changeTokenBalance(
                ymwk,
                addr1,
                amount
            );
        ...
        it("mint_fail_2", async function () {
            ...
            await expect(ymwk.connect(addr1).mint(ethers.constants.AddressZero, amount)).to.be.reverted;
        });
    });
    ...
    describe("update_mining_parameters", function () {
        ...
        it("update_mining_parameters_success_1", async function () {
            ...
            for(let i = 1; i < 236; i++) {
                ...
                const supply = BigInt(currentRate.toString()) * 3600n * 24n * 365n / (10n ** 18n);
                if(i < 7) {
            ...
            const mintableIn235 = await ymwk.mintable_in_timeframe(startTime, endTime);

            // console.log(availableSupplyAtLast.toString(), mintableIn235.toString())
            await expect(availableSupplyAtLast).to.be.above(ethers.utils.parseEther("999999999"));
            await expect(availableSupplyAtLast).to.be.below(ethers.utils.parseEther("1000000000"));
            await expect(mintableIn235).to.be.above(ethers.utils.parseEther("549999820"));
            await expect(mintableIn235).to.be.below(ethers.utils.parseEther("549999823"));
        });
    });
});
