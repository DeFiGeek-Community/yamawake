...
describe("FeePool", function () {
  ...
  describe("withdrawEther", function () {
    ...
    it("withdrawEther_success_1", async function () {
      const {feePool, owner} = await loadFixture(deployFactoryAndFeePoolFixture);
      sendEther(feePool.address, "1", owner);
      expect(await feePool.connect(owner).withdrawEther(owner.address)).to.changeEtherBalances(
        [feePool, owner],
    ...
    it("withdrawEther_fail_1", async function () {
        const {feePool, owner} = await loadFixture(deployFactoryAndFeePoolFixture);
        sendEther(feePool.address, "1", owner);
        expect(feePool.connect(owner).withdrawEther(ethers.constants.AddressZero)).to.be.revertedWith(
            "Don't discard treasury!"
        );
    });

      // オーナー以外の手数料回収
    it("withdrawEther_fail_2", async function () {
        const {feePool, owner, addr1} = await loadFixture(deployFactoryAndFeePoolFixture);
        sendEther(feePool.address, "1", owner);
  ...
  describe("withdrawToken", function () {
    ...
    it("withdrawToken_success_1", async function () {
      ...
      expect(await feePool.connect(owner).withdrawToken(owner.address, [token.address])).to.changeTokenBalances(
        token,
        [feePool, owner],
        [ethers.utils.parseEther("1"), `-${ethers.utils.parseEther("1")}`]
      );
    ...
    it("withdrawToken_fail_1", async function () {
        ...
        const amount = ethers.utils.parseEther("1");
        await token.transfer(feePool.address, amount);
        expect(feePool.connect(owner).withdrawToken(ethers.constants.AddressZero)).to.be.revertedWith(
            "Don't discard treasury!"
        );
    });
